...
class Settings(BaseSettings):
    ...
    manga_storage_paths: List[Path] = Field(
        ...
        description="List of manga storage paths to scan"
    )
    
    # External APIs
    mangadx_api_url: str = Field(
        default="https://api.mangadx.org",
        description="MangaDx API base URL"
    )
    mangadx_rate_limit_per_minute: int = Field(
        default=60,
        description="MangaDx API rate limit per minute"
    )
