...
class MetricsCollector:
    ...
    def record_polling_end(
        ...
        with self._lock:
            ...
            if metric.duration_ms:
                self.performance_buckets[f"polling.{metric.operation_type}.duration"].append(metric.duration_ms)
                self.performance_buckets["polling.total.duration"].append(metric.duration_ms)
            
            # Update gauges
            self.gauges[f"polling.{metric.operation_type}.last_duration_ms"] = metric.duration_ms or 0
            self.gauges["polling.chapters_processed.total"] = self.gauges.get("polling.chapters_processed.total", 0) + chapters_processed
            self.gauges["polling.notifications_created.total"] = self.gauges.get("polling.notifications_created.total", 0) + notifications_created
        
        logger.debug(f"Completed polling operation: {metric.operation_type} (duration: {metric.duration_ms:.2f}ms, success: {success})")
    ...
    def record_api_request_start(self, endpoint: str, method: str, user_id: Optional[UUID] = None) -> APIMetric:
    ...
    def record_api_request_end(
        ...
        with self._lock:
            ...
            # Track performance
            if metric.duration_ms:
                self.performance_buckets[f"api.{metric.endpoint}.duration"].append(metric.duration_ms)
            ...
            self.gauges["api.total_response_bytes"] = self.gauges.get("api.total_response_bytes", 0) + response_size_bytes
    ...
    def get_system_metrics(self) -> SystemMetrics:
        ...
        with self._lock:
            ...
            if self.polling_metrics:
                durations = [m.duration_ms for m in self.polling_metrics if m.duration_ms is not None]
                if durations:
    ...
    def get_polling_metrics(self, operation_type: Optional[str] = None, limit: int = 100) -> List[Dict[str, Any]]:
