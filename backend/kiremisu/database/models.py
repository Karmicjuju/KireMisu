...
from sqlalchemy import ARRAY, DateTime, String, Text, Boolean, Integer, JSON
...
class Chapter(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    series_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True)
...
class Annotation(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    chapter_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True)
    ...
    # Metadata
    annotation_type: Mapped[str] = mapped_column(String(50), nullable=False, default="note")  # note, bookmark, highlight
...
class JobQueue(Base):
    ...
    status: Mapped[str] = mapped_column(String(20), nullable=False, default="pending", index=True)  # pending, running, completed, failed
    ...
    # Scheduling
    scheduled_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow, index=True)
