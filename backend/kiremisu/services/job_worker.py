...
class JobWorker:
    ...
    async def _handle_job_failure(self, db: AsyncSession, job: JobQueue, error_msg: str):
        ...
        try:
            ...
            if should_retry:
                ...
                logger.info(f"Job {job.id} will be retried (attempt {job.retry_count + 1}/{job.max_retries})")
            else:
                # Mark as failed
                await self._update_job_status(
                    db,
    ...
    async def _execute_library_scan(self, db: AsyncSession, job: JobQueue) -> Dict[str, Any]:
        ...
        stats = await self.importer.scan_library_paths(
            db=db,
...
class JobWorkerRunner:
    ...
    def __init__(self, db_session_factory, poll_interval_seconds: int = 10, max_concurrent_jobs: int = 3):
    ...
    async def start(self):
        ...
        logger.info(f"Started background job worker (poll interval: {self.poll_interval}s, max concurrent: {self.max_concurrent_jobs})")
