...
class FileOperationError(Exception):
    ...
    def __init__(self, message: str, operation_id: Optional[UUID] = None, details: Optional[Dict] = None):
...
class FileOperationService:
    ...
    async def create_operation(
        ...
        if not os.path.exists(request.source_path):
            raise FileOperationError(
                f"Source path does not exist: {request.source_path}",
                operation_id=operation.id
            )

        if not os.access(request.source_path, os.R_OK):
            raise FileOperationError(
                f"Source path is not readable: {request.source_path}",
                operation_id=operation.id
            )
        ...
        if request.operation_type in ["rename", "move"] and request.target_path:
            ...
            if not target_parent.exists():
                raise FileOperationError(
                    f"Target directory does not exist: {target_parent}",
                    operation_id=operation.id
                )
            
            if not os.access(str(target_parent), os.W_OK):
                raise FileOperationError(
                    f"Target directory is not writable: {target_parent}",
                    operation_id=operation.id
                )
    ...
    async def validate_operation(
        ...
        result = await db.execute(
            select(FileOperation).where(FileOperation.id == operation_id)
        )
    ...
    async def rollback_operation(
        ...
        if not operation.backup_path:
            raise FileOperationError(
                "Cannot rollback operation without backup",
                operation_id=operation.id
            )
    ...
    async def _validate_file_system_operation(
        ...
        if operation.operation_type in ["rename", "move"] and operation.target_path:
            ...
            if not target_path.parent.exists():
                validation_result.errors.append(f"Target directory does not exist: {target_path.parent}")
                validation_result.is_valid = False
            elif not os.access(str(target_path.parent), os.W_OK):
                validation_result.errors.append(f"Target directory is not writable: {target_path.parent}")
                validation_result.is_valid = False
        ...
        if operation.operation_metadata.get("create_backup", True):
            ...
            if size_mb > available_space * 0.8:  # Use max 80% of available space
                validation_result.warnings.append(f"Limited disk space for backup: {size_mb:.1f}MB needed, {available_space:.1f}MB available")
                validation_result.risk_level = "medium"
    ...
    async def _validate_database_consistency(
        ...
        if not affected_series and not affected_chapters:
            validation_result.warnings.append("No database records found for this path - may be untracked files")
            return
    ...
    async def _assess_operation_risks(
        ...
        # Large files
        if validation_result.estimated_disk_usage_mb and validation_result.estimated_disk_usage_mb > 1000:
            risk_factors += 1
    ...
    async def _find_affected_records(
        ...
        chapters_result = await db.execute(
            select(Chapter).where(
                or_(
                    Chapter.file_path == source_path,
                    Chapter.file_path.like(f"{source_path}%")
                )
            )
        )
    ...
    async def get_operation(self, db: AsyncSession, operation_id: UUID) -> Optional[FileOperationResponse]:
