...
class MangaDxImportService:
    ...
    async def _convert_mangadx_to_manga_info(
        self, mangadx_response: MangaDxMangaResponse
    ) -> MangaDxMangaInfo:
        """
    ...
    def _calculate_match_confidence(
        ...
        if local_series.author and mangadx_info.author:
            author_similarity = self._calculate_title_similarity(
                local_series.author, mangadx_info.author
            )
        ...
        if local_series.artist and mangadx_info.artist:
            artist_similarity = self._calculate_title_similarity(
                local_series.artist, mangadx_info.artist
            )
        ...
        if local_series.genres and mangadx_info.genres:
            ...
            if local_genres_set and mangadx_genres_set:
                genre_overlap = len(local_genres_set & mangadx_genres_set) / len(
                    local_genres_set | mangadx_genres_set
                )
    ...
    async def search_and_match_series(
        ...
        logger.info(
            f"Searching MangaDx for matches to series '{local_series.title_primary}' using query: '{query}'"
        )
        ...
        try:
            ...
            if auto_select_best_match and candidates:
                ...
                if best_candidate.confidence_score >= confidence_threshold:
                    ...
                    logger.info(
                        f"Auto-selected MangaDx match: {best_candidate.mangadx_info.title} (confidence: {best_candidate.confidence_score:.2f})"
                    )
        ...
        try:
            ...
            if import_request.target_series_id:
                ...
                result = await db_session.execute(
                    select(Series).where(Series.id == import_request.target_series_id)
                )
                series = result.scalar_one_or_none()

                if not series:
                    raise MangaDxImportError(
                        f"Target series not found: {import_request.target_series_id}"
                    )
            ...
            series.source_metadata = {
                "mangadx": {
                    ...
                    "mangadx_updated_at": mangadx_info.mangadx_updated_at.isoformat()
                    if mangadx_info.mangadx_updated_at
                    else None,
                }
            }
            ...
            if import_request.import_cover_art and mangadx_info.cover_art_url:
                cover_path = await self._download_cover_art(
                    mangadx_info.cover_art_url, mangadx_info.id
                )
                if cover_path:
            ...
            logger.info(
                f"Successfully {operation} series {series.id} with MangaDx metadata (duration: {import_duration_ms}ms)"
            )
