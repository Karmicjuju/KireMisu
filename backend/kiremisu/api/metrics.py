...
@router.get("/system", response_model=SystemMetricsResponse, responses={
        ...
        return SystemMetricsResponse(
            ...
            uptime_info={
                ...
                "total_metrics_collected": (
                    system_metrics.total_polling_operations + 
                    system_metrics.total_api_requests
                )
            }
        )
...
@router.get("/polling", response_model=PollingMetricsResponse, responses={
    500: {"model": ErrorResponse, "description": "Internal server error"}
})
...
@router.get("/performance/{metric_name}", response_model=PerformanceStatsResponse, responses={
...
@router.get("/watching", response_model=dict, responses={
    500: {"model": ErrorResponse, "description": "Internal server error"}
})
async def get_watching_metrics() -> dict:
    ...
    try:
        system_metrics = metrics_collector.get_system_metrics()
        watching_polling_metrics = metrics_collector.get_polling_metrics("watching_schedule_checks", 50)
...
@router.get("/counters", response_model=dict, responses={
    500: {"model": ErrorResponse, "description": "Internal server error"}
})
...
@router.post("/reset", response_model=dict, responses={
...
async def get_metrics_health() -> dict:
    ...
    try:
        ...
        with metrics_collector._lock:
            polling_utilization = len(metrics_collector.polling_metrics) / metrics_collector.max_history_size
            api_utilization = len(metrics_collector.api_metrics) / metrics_collector.max_history_size
