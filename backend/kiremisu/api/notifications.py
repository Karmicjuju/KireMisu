...
async def get_notification_count(db: AsyncSession = Depends(get_db)) -> dict:
...
async def get_notification_stats(db: AsyncSession = Depends(get_db)) -> NotificationStatsResponse:
