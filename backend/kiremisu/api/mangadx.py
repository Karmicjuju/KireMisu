...
async def search_manga(
    ...
    """
    logger.info(
        f"Searching MangaDx: title='{search_request.title}', author='{search_request.author}', limit={search_request.limit}"
    )
        ...
        for manga in search_response.data:
            ...
            for relationship in manga.relationships:
                if relationship.get("type") == "cover_art":
                    ...
                    if cover_filename:
                        cover_art_url = await client.get_cover_art_url(
                            manga.id, cover_filename, size="512"
                        )
                    break
...
async def bulk_import_manga_metadata(
    ...
    """
    logger.info(
        f"Scheduling bulk import of {len(bulk_request.import_requests)} manga metadata records"
    )
...
async def import_and_download_manga(
    ...
    try:
        ...
        if download_chapters and import_result.success and import_result.series_id:
            try:
                ...
                job_result = await db_session.execute(
                    select(JobQueue).where(JobQueue.id == download_job_id)
                )
        ...
        raise _handle_mangadx_error(
            e, f"import and download manga {import_request.mangadx_id}"
        ) from e
    finally:
        await download_service.cleanup()
