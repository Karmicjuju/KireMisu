...
def validate_path_input(path: str, client_ip: str = "unknown") -> None:
    ...
    security_logger.info(
        f"Path access attempt: path='{path}' client_ip='{client_ip}'"
    )
    
    # Check path length
    if len(path) > MAX_PATH_LENGTH:
        security_logger.warning(
            f"Path too long: length={len(path)} client_ip='{client_ip}'"
        )
        raise HTTPException(
            status_code=400, 
    ...
    for pattern in DANGEROUS_PATTERNS:
        if re.search(pattern, path, re.IGNORECASE):
            ...
            raise HTTPException(
                status_code=400,
                detail="Invalid path format detected."
            )
...
def validate_path_security(requested_path: Path, client_ip: str = "unknown") -> Path:
    ...
    try:
        ...
        common_path = Path(os.path.commonpath([
            str(MANGA_STORAGE_BASE), 
            str(requested_path)
        ]))
        ...
        raise HTTPException(
            status_code=403,
            detail="Access denied. Invalid path structure."
        )
...
async def browse_directory(
    ...
    page_size: int = Query(DEFAULT_PAGE_SIZE, ge=1, le=MAX_ITEMS_PER_PAGE, description="Items per page")
) -> DirectoryListing:
    """
    ...
    try:
        ...
        if not validated_path.exists():
            security_logger.info(f"Directory not found: path='{validated_path}' client_ip='{client_ip}'")
            raise HTTPException(status_code=404, detail="Directory not found")
        
        if not validated_path.is_dir():
            security_logger.warning(f"Path is not directory: path='{validated_path}' client_ip='{client_ip}'")
            raise HTTPException(status_code=400, detail="Path is not a directory")
        
        if not os.access(validated_path, os.R_OK):
            security_logger.warning(f"Directory not readable: path='{validated_path}' client_ip='{client_ip}'")
        ...
        try:
            for item in validated_path.iterdir():
                ...
                try:
                    ...
                    all_items.append(DirectoryItem(
                        name=item.name,
                        path=str(item),
            ...
            security_logger.warning(f"Permission denied listing: path='{validated_path}' client_ip='{client_ip}'")
        ...
        security_logger.error(
            f"Unexpected error in browse_directory: {str(e)} "
...
async def validate_path(
    ...
    try:
        ...
        security_logger.error(
            f"Unexpected error in validate_path: {str(e)} "
            f"path='{path}' client_ip='{client_ip}'"
        )
        return {
            "exists": False,
