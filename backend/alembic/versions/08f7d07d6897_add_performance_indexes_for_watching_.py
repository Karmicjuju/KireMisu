...
def upgrade() -> None:
    ...
    result = connection.execute(text(
        "SELECT indexname FROM pg_indexes WHERE indexname = 'ix_series_watching_enabled'"
    ))
    ...
    if not result.fetchone():
        op.create_index(
            'ix_notifications_series_id_is_read',
            'notifications',
            ['series_id', 'is_read']
        )
    ...
    result = connection.execute(text(
        "SELECT indexname FROM pg_indexes WHERE indexname = 'ix_notifications_is_read'"
    ))
    if not result.fetchone():
        op.create_index(
