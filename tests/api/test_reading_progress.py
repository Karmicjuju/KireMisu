...
class TestReadingProgressAPI:
    ...
    async def test_update_chapter_progress_completion(self, client: AsyncClient, sample_chapters):
    ...
    async def test_update_chapter_progress_explicit_completion(self, client: AsyncClient, sample_chapters):
    ...
    async def test_update_chapter_progress_invalid_chapter(self, client: AsyncClient):
    ...
    async def test_update_chapter_progress_invalid_page(self, client: AsyncClient, sample_chapters):
    ...
    async def test_toggle_chapter_read_status(self, client: AsyncClient, sample_chapters):
        """Test toggling chapter read status."""
        chapter = sample_chapters[0]
        
        # Mark as read
        response = await client.post(f"/reading-progress/chapters/{chapter.id}/mark-read")
    ...
    async def test_mark_chapter_unread(self, client: AsyncClient, sample_chapters):
        """Test marking chapter as unread."""
        chapter = sample_chapters[0]
        
        # First mark as read
        await client.post(f"/reading-progress/chapters/{chapter.id}/mark-read")
        
        # Then specifically mark as unread
        response = await client.post(f"/reading-progress/chapters/{chapter.id}/mark-unread")
    ...
    async def test_mark_chapter_unread_already_unread(self, client: AsyncClient, sample_chapters):
    ...
    async def test_get_chapter_progress(self, client: AsyncClient, sample_chapters):
        ...
        # Then get progress
        response = await client.get(f"/reading-progress/chapters/{chapter.id}/progress")
    ...
    async def test_get_chapter_progress_not_found(self, client: AsyncClient):
        """Test getting progress for non-existent chapter."""
        invalid_id = uuid4()
        
        response = await client.get(f"/reading-progress/chapters/{invalid_id}/progress")
        
        assert response.status_code == 404

    async def test_get_series_progress(self, client: AsyncClient, sample_series, sample_chapters):
        """Test getting series reading progress."""
        # Mark first chapter as read
        await client.post(f"/reading-progress/chapters/{sample_chapters[0].id}/mark-read")
        
        response = await client.get(f"/reading-progress/series/{sample_series.id}/stats")
    ...
    async def test_get_series_progress_not_found(self, client: AsyncClient):
        ...
        response = await client.get(f"/reading-progress/series/{invalid_id}/stats")
        
        assert response.status_code == 404

    async def test_mark_series_read(self, client: AsyncClient, sample_series, sample_chapters):
        """Test marking entire series as read."""
        response = await client.post(f"/reading-progress/series/{sample_series.id}/mark-read")
        ...
        progress_response = await client.get(f"/reading-progress/series/{sample_series.id}/stats")
        progress_data = progress_response.json()
        assert progress_data["read_chapters"] == 3
        assert progress_data["progress_percentage"] == 100.0

    async def test_mark_series_unread(self, client: AsyncClient, sample_series, sample_chapters):
        """Test marking entire series as unread."""
        # First mark as read
        await client.post(f"/reading-progress/series/{sample_series.id}/mark-read")
        
        # Then mark as unread
        response = await client.post(f"/reading-progress/series/{sample_series.id}/mark-unread")
    ...
    async def test_mark_series_not_found(self, client: AsyncClient):
        """Test marking non-existent series."""
        invalid_id = uuid4()
        
        response = await client.post(f"/reading-progress/series/{invalid_id}/mark-read")
        assert response.status_code == 404
        
        response = await client.post(f"/reading-progress/series/{invalid_id}/mark-unread")
        assert response.status_code == 404

    async def test_get_user_reading_stats(self, client: AsyncClient, sample_chapters):
        ...
        response = await client.get("/reading-progress/user/stats")
    ...
    async def test_api_validation_errors(self, client: AsyncClient, sample_chapters):
    ...
    async def test_progress_tracking_integration(self, client: AsyncClient, sample_chapters):
    ...
    async def test_series_statistics_updates(self, client: AsyncClient, sample_series, sample_chapters):
        ...
        await client.post(f"/reading-progress/chapters/{sample_chapters[1].id}/mark-read")
