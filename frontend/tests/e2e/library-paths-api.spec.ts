...
test.describe('Library Paths API Integration', () => {
  ...
  test('should display existing library paths', async ({ page }) => {
    ...
    await expect(page.getByText('Last scan: 8/3/2025')).toBeVisible();
    await expect(page.getByText('Enabled')).toBeVisible();
    
    // Should show edit and delete buttons
    await expect(page.getByRole('button', { name: 'Edit' })).toBeVisible();
    await expect(page.getByRole('button').filter({ hasText: '' })).toBeVisible(); // Trash icon
  });
  ...
  test('should successfully edit a library path', async ({ page }) => {
    ...
    // Submit the form
    await page.getByRole('button', { name: 'Update Path' }).click();
  ...
  test('should handle network errors', async ({ page }) => {
    ...
    await expect(page.getByText('Failed to load library paths.')).toBeVisible();
  });
});