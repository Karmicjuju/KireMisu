...
class R1Validator:
    ...
    def log_result(self, test_name: str, success: bool, details: str = "", error: str = ""):
        """Log a test result."""
        self.results.append(
            {"test": test_name, "success": success, "details": details, "error": error}
        )
        ...
        try:
            ...
            if backend_healthy and frontend_accessible:
                ...
                self.log_result(
                    "System Health Check",
                    False,
                    f"Backend: {backend_healthy}, Frontend: {frontend_accessible}",
                )
    ...
    def test_series_discovery(self):
        ...
        try:
            ...
            if os.path.exists(manga_storage):
                series_dirs = [
                    d
                    for d in os.listdir(manga_storage)
                    if os.path.isdir(os.path.join(manga_storage, d))
                ]
                if series_dirs:
                    self.log_result(
                        "Series Discovery (Filesystem)",
                        True,
                        f"Found {len(series_dirs)} series directories",
                    )
                else:
                    self.log_result(
                        "Series Discovery (Filesystem)", False, "No series directories found"
                    )
            else:
                self.log_result(
                    "Series Discovery (Filesystem)", False, "Manga storage directory not found"
                )
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if pages > 0:
                            ...
                            chapter_found = True
                            self.log_result(
                                "Chapter API", True, f"Found chapter with {pages} pages"
                            )
                            break
                        ...
                        self.log_result(
                            "Chapter API", False, f"Unexpected status: {response.status_code}"
                        )
                        break
                except:
                    continue
    ...
    def test_page_streaming(self):
        ...
        try:
            ...
            if response.status_code != 200:
                self.log_result(
                    "Page Streaming", False, f"Pages info failed: HTTP {response.status_code}"
                )
                return
            ...
            if page_response.status_code == 200:
                ...
                self.log_result(
                    "Page Streaming",
                    False,
                    f"Page streaming failed: HTTP {page_response.status_code}",
                )
            ...
            try:
                ...
                if response.status_code == 400:
                    ...
                    if "../../../etc/passwd" not in error_msg:
                        self.log_result(
                            "Path Validation & Error Sanitization",
                            True,
                    ...
                    self.log_result(
                        "Path Validation",
                        False,
                        f"Malicious path not rejected: HTTP {response.status_code}",
                    )
            except Exception as e:
